...
export class SendNoticeCreatedAlertEventHandler implements IEventHandler<NoticeCreatedEvent> {
  ...
  private identify(event: NoticeCreatedEvent): NoticeCreatedEvent {
    return event;
  }

  private verify(event: NoticeCreatedEvent): void {
    const notice = event.notice;
    if (!notice) {
      throw new Error('공지 생성 이벤트를 확인할 수 없습니다.');
    }
  }

  private async process(event: NoticeCreatedEvent): Promise<void> {
    const { notice } = event;
    const now = new Date();
