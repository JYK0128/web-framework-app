...
import type { IOAuthChannel, OAuthContext, OAuthProfile, OAuthProvider, OAuthToken } from '#/common/services/oauth/oauth.interface';
...
export abstract class BaseOAuthChannel implements IOAuthChannel {
  ...
  createAuthorizeUrl(state: string, context: OAuthContext): string {
    const clientId = context.credentials?.clientId || this.getDefaultClientId();
    const callbackUrl = this.getCallbackUrl(context.frontendUrl);
  ...
  async exchangeCode(code: string, context: OAuthContext): Promise<OAuthToken | null> {
    try {
      const clientId = context.credentials?.clientId || this.getDefaultClientId();
      const clientSecret = context.credentials?.clientSecret || this.getDefaultClientSecret();
      const callbackUrl = this.getCallbackUrl(context.frontendUrl);
  ...
  protected getDefaultClientSecret(): string {
    return '';
  }

  protected getCallbackUrl(frontendUrl: string): string {
    return new URL(this.callbackRoute, `${frontendUrl.replace(/\/$/, '')}/`).toString();
  }
}
