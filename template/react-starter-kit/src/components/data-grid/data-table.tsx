...
const ROW_HEIGHT = 48;    // matches h-12 (DataTableRow cells)
...
// ─── DataTable ─────────────────────────────────────────────────────────────
export function DataTable<TData>({ table, hideHeader = false }: DataTableProps<TData>) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <DataTableBody table={table} hideHeader={hideHeader} gridTemplateColumns={gridTemplateColumns} containerRef={containerRef} />
      </table>
    </div>
  );
}
...
export function DataTableBody<TData>({
  ...
  gridTemplateColumns,
  containerRef,
}: Pick<DataTableProps<TData>, 'table' | 'hideHeader'> & {
  ...
  const centerRows = table.getCenterRows().filter((r) => !topRowIds.has(r.id));
...
// ─── DataTableHead ──────────────────────────────────────────────────────────
function DataTableHead<TData>({ header, draggingColumn, onDragStart, onDragEnter, onDragEnd }: {
...
function DataTableResizeHandle<TData>({ header }: { header: ReturnType<TanStackTable<TData>['getHeaderGroups']>[number]['headers'][number] }) {
