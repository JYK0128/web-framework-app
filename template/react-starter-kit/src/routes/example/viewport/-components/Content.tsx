    ...
    <AppContent className="scroll-y min-w-0 min-h-0 space-y-3.5 bg-muted/20 p-4">
      <div className="space-y-1">
        ...
        <Input
          ...
          autoComplete="off"
          enterKeyHint="next"
          className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-base text-foreground shadow-2xs transition-all placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
        />
      </div>

      <Card className="rounded-2xl border-border/60 shadow-2xs">
        <CardContent className="space-y-3.5 p-4">
          <div className="flex items-center justify-between rounded-xl border border-border/30 bg-muted/50 p-2.5 text-xs">
            <span className="font-medium text-muted-foreground">🔄 화면 회전 단축키</span>
            <kbd className="rounded-md border border-border/60 bg-background px-2 py-0.5 font-mono text-xs font-semibold text-foreground shadow-2xs">
              ⌘ + ⬅️ / ➡️
            </kbd>
          </div>
      ...
      <Card className="rounded-2xl border-border/60 shadow-2xs">
        <CardContent className="space-y-3.5 p-4">
          ...
          <Button id="btn-focus-input" size="lg" onClick={onFocusInput} className="w-full justify-center rounded-xl font-medium">
            ⌨️ 버튼 클릭으로 입력창 포커스하기
          </Button>
          ...
          <div className="space-y-1.5 border-t border-border/30 pt-1">
            <div className="mb-1 flex items-center justify-between">
              ...
              <Badge variant="outline" className="border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-xs text-emerald-700">
                AOS 전용
              </Badge>
            </div>
            <ViewportOptionGroup
      ...
      <Card className="rounded-2xl border-border/60 shadow-2xs">
        <CardContent className="space-y-3 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            <Button size="sm" variant="outline" onClick={onOpenDialog} className="w-full justify-center rounded-xl text-xs font-medium">
              💬 Dialog 모달 열기
            </Button>
            <Button
              size="sm"
...
function ViewportOptionGroup<T extends string>({ options, value, onChange }: ViewportOptionGroupProps<T>) {
  return (
    <ToggleGroup
      ...
      size="sm"
      spacing={0}
      className="grid w-full grid-cols-[repeat(auto-fit,minmax(0,1fr))] overflow-hidden rounded-xl border border-border/40 bg-muted/80 shadow-inner"
    >
      {options.map(({ id, label }) => (
        <ToggleGroupItem
          key={id}
          value={id}
          className="min-h-8 rounded-none border-0 px-1 text-center text-xs leading-tight whitespace-pre-line first:rounded-l-lg last:rounded-r-lg"
        >
          {label}
        </ToggleGroupItem>
      ))}
