...
function PermissionPageComponent() {
  ...
  const roles = useMemo<RoleItemDto[]>(() => rolesData?.items ?? [], [rolesData?.items]);

  const { data: resourcesData } = useResourcesControllerGetResources();
  const resources: ResourceItemDto[] = resourcesData?.items ?? [];
  ...
  const handleDeleteResource = useCallback(async (resource: ResourceItemDto) => {
  ...
  const handleDeleteRole = async (role: RoleItemDto, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <PageSection icon="lock" title={t('permissionManagement.title')} description={t('permissionManagement.description')}>
      <PageSection.Content className="
        grid p-2 scroll-y
        lg:overflow-hidden lg:grid-rows-[minmax(0,1fr)]
      "
      >
        <div className="
          grid grid-cols-1
