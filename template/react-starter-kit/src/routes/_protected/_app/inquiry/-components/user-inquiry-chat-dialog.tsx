...
export function UserInquiryChatDialog({
  ...
  const messageMutation = useInquiriesControllerCreateInquiryMessage();
  const updateMutation = useInquiriesControllerUpdateInquiry();
  ...
  useEffect(() => {
    ...
    const handleConnect = () => {
      ...
      joinInquiryRoom(socket, inquiryId, false, (success) => {
        if (disposed) return;
  ...
  return (
    <InquiryChatView
      ...
      isClosing={updateMutation.isPending}
    />
  );
}
