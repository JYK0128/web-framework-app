          ...
          >
            <div className="scroll-y grid gap-4 py-2 pr-1">
              <form.AppField name="providerId">
                {(field) => (
                  <field.Input
                    label={t('systemManagement.oauth.customId')}
              ...
              <form.AppField name="providerName">{(field) => <field.Input label={t('systemManagement.oauth.customName')} placeholder="예: Okta SSO" required />}</form.AppField>
              <form.AppField name="providerScope">{(field) => <field.Input label={t('systemManagement.oauth.scope')} placeholder="openid profile email" />}</form.AppField>
            ...
            <DialogFooter className="pt-3">
              <Button type="button" variant="outline" onClick={() => close?.(null)}>{t('app.dialog.cancel')}</Button>
              <FormSubmit>
                <Plus className="size-4" />
                {t('systemManagement.oauth.addProvider')}
              </FormSubmit>
            </DialogFooter>
          </FormLayout>
        </form.AppForm>
