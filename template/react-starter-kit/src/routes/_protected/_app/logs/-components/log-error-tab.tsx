  ...
  return (
    ...
    >
      ...
      {error
        ? (
          <section className="grid h-full grid-cols-1">
            <nav className="scroll-x flex gap-1 border-b">
              {[
                { key: 'message' as const, label: t('logs.detail.errorMessage'), value: error.message },
                { key: 'details' as const, label: t('logs.detail.validationDetails'), value: error.details },
                { key: 'sql' as const, label: t('logs.detail.sqlStatement'), value: error.sql },
                { key: 'stack' as const, label: t('logs.detail.stackTrace'), value: error.stack },
              ].map((item) => (
                <button
                  key={item.key}
                  type="button"
            ...
            <DiagnosticBlock
              title={[
                { key: 'message' as const, label: t('logs.detail.errorMessage'), content: error.message || t('logs.detail.unknownServerError') },
                { key: 'details' as const, label: t('logs.detail.validationDetails'), content: formatDiagnosticValue(error.details) },
                { key: 'sql' as const, label: t('logs.detail.sqlStatement'), content: formatDiagnosticValue(error.sql) },
                { key: 'stack' as const, label: t('logs.detail.stackTrace'), content: formatDiagnosticValue(error.stack) },
              ].find((item) => item.key === selectedField)?.label ?? ''}
              content={[
                { key: 'message' as const, content: error.message || t('logs.detail.unknownServerError') },
              ...
              onCopy={() => {
                if (selectedField === 'message') {
                  handleCopy(error.message || t('logs.detail.unknownServerError'));
                  return;
                }
        ...
        : <p className="text-sm text-muted-foreground italic">{t('logs.detail.noErrorDetail')}</p>}
    </TabsContent>
  );
}
