...
interface LogResponseTabProps {
  log: LogItemDto
}

export function LogResponseTab({ log }: LogResponseTabProps) {
  const { t } = useI18n();
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted-foreground">{t('logs.detail.responsePayloadJson')}</span>
        {hasBody && (
          <Button variant="outline" size="sm" className="h-7 gap-1 text-xs" onClick={handleCopy}>
            <Copy className="size-3" />
            {t('logs.detail.copyJson')}
          </Button>
        )}
      </div>
      <pre className="
