...
import { logMethodVariants, toLogMethodVariant } from '#/routes/_protected/_app/logs/-configs/log.config';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      : ''}
          ...
          <DialogTitle className="sr-only">{t('logs.detail.title')}</DialogTitle>
          <DialogDescription className="sr-only">{t('logs.detail.title')}</DialogDescription>
          ...
          >
            <TabsList className="grid h-10 w-full grid-cols-4">
              <TabsTrigger value="general" className="gap-1.5 text-xs">
                ...
                <span className="truncate">{t('logs.detail.general')}</span>
              </TabsTrigger>
              <TabsTrigger value="request" className="gap-1.5 text-xs">
                <Server className="size-3.5 shrink-0" />
                <span className="truncate">{t('logs.detail.requestPayload')}</span>
              </TabsTrigger>
              <TabsTrigger value="response" className="gap-1.5 text-xs">
                <Globe className="size-3.5 shrink-0" />
                <span className="truncate">{t('logs.detail.responsePayload')}</span>
              </TabsTrigger>
              <TabsTrigger
                value="error"
                disabled={!isError}
              ...
              >
                ...
                <span className="truncate">{t('logs.detail.error')}</span>
              </TabsTrigger>
            </TabsList>
