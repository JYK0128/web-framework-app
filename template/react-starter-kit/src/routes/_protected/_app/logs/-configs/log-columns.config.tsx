...
export function createLogColumns({ i18n, onSelectLog }: LogColumnDependencies) {
  ...
  return [
    columnHelper.accessor('createdAt', {
      header: translate('logs.columns.timestamp'),
      size: 170,
      cell: ({ getValue }) => (
    ...
    columnHelper.accessor('method', {
      header: translate('logs.columns.method'),
      size: 100,
    ...
    columnHelper.accessor('statusCode', {
      header: translate('logs.columns.status'),
      size: 100,
      enableColumnFilter: true,
      meta: {
        filterType: 'faceted',
        filterOptions: STATUS_OPTIONS.filter((s) => s !== 'ALL').map((s) => ({
          label: translate(`logs.filters.statuses.${s}`),
          value: s,
        })),
      },
      filterFn: (row, id, value) => {
    ...
    columnHelper.accessor('url', {
      header: translate('logs.columns.url'),
      size: 320,
      cell: ({ row }) => (
    ...
    columnHelper.accessor('duration', {
      header: translate('logs.columns.duration'),
      size: 100,
      cell: ({ getValue }) => {
    ...
    columnHelper.accessor('ip', {
      header: translate('logs.columns.ip'),
      size: 130,
      cell: ({ getValue }) => (
    ...
    columnHelper.accessor('emailHash', {
      header: translate('logs.columns.user'),
      size: 120,
      cell: ({ getValue }) => {
    ...
    columnHelper.accessor('id', {
      id: 'actions',
      header: translate('logs.manage'),
      enableSorting: false,
      size: 80,
      cell: ({ row }) => (
        <div className="text-right" onClick={(event) => event.stopPropagation()}>
          <Button variant="ghost" size="icon" title={translate('logs.columns.detail')} aria-label={translate('logs.columns.detail')} onClick={() => onSelectLog(row.original)}>
            <Eye className="
