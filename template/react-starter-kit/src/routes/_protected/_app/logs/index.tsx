...
import { logsControllerGetLogs, logsControllerGetStats } from '#/.generated/api/endpoints/logs/logs';
...
export const Route = createFileRoute('/_protected/_app/logs/')({
  beforeLoad: ({ context }) => {
...
function LogsPage() {
  ...
  const {
    ...
    queryFn: ({ pageParam }) => {
      ...
      return logsControllerGetLogs(params);
    },
  ...
  const {
    ...
    queryFn: () => {
      ...
      return logsControllerGetStats({ startDate });
    },
  });
  ...
  return (
    <PageSection icon="activity" title={t('logs.title')} description={t('logs.description')}>
      <PageSection.Actions>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium">
            {isLive ? t('logs.liveStreaming') : t('logs.paused')}
          </span>
          <Switch
            checked={isLive}
            onCheckedChange={setIsLive}
            aria-label={t('logs.toggleStream')}
          />
        </div>
      </PageSection.Actions>
          ...
          >
            <SectionCard.Actions>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">{t('logs.statsPeriod')}</span>
                <Select
                  items={TIME_RANGE_OPTIONS.map((r) => ({ label: t(`logs.filters.timeRanges.${r}`), value: r }))}
                  value={timeRange}
                  ...
                  <SelectContent>
                    {TIME_RANGE_OPTIONS.map((range) => (
                      <SelectItem key={range} value={range}>
                        {t(`logs.filters.timeRanges.${range}`)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <SectionCard textSize="sm">
            <SectionCard.Content>
              <div className="grid h-full grid-rows-[auto_1fr_auto]">
                <DataGridToolbar
                  table={table}
                  searchPlaceholder={t('logs.filters.searchPlaceholder')}
                  onReset={() => {
                    setColumnFilters([]);
                    setSearchKeyword('');
                ...
                >
                  <span>
                    {t('logs.loadedCount', { count: mergedLogs.length })}
                  </span>
                  ...
                  <span className="flex items-center gap-3">
                    ...
                    {!hasNextPage && mergedLogs.length > 0 && !isFetchingNextPage && (
                      <span>
                        {t('logs.allLogsLoaded')}
                      </span>
                    )}

                    {effectiveTotalCount > 0 && (
                      <span>
                        {t('logs.totalCount', { count: effectiveTotalCount.toLocaleString() })}
                      </span>
                    )}
                  </span>
                </div>
