...
type ScrollIntent =
  | { action: 'idle' }
  | { action: 'align-content' }                               // ① 본문: 1/3 정렬
...
export function initVirtualKeyboard(): () => void {
  ...
  let state: State = {
    ...
    snapshot: {
      ...
      appHeight: toPx(initialViewport.height),
    },
    scroll: { action: 'idle' },
  };
  ...
  function planScrollIntent(
    ...
    // ① 본문 포커스: align-content
    if (nextRegion === 'content') {
      if (state.keyboard === 'closed' || state.keyboard === 'closing') {
    ...
    if (nextRegion === 'footer') {
      ...
      if (contentView) {
        const bottomDistance =
          state.scroll.action === 'preserve-footer' || state.scroll.action === 'restore-footer'
            ? state.scroll.bottomDistance
            : measureView(contentView).bottomDistance;
        state.scroll = { action: 'preserve-footer', bottomDistance };
      } else {
        state.scroll = { action: 'idle' };
      }
      return;
    ...
    // ② 본문 blur: 가상 키보드가 닫힐 때 본문 스크롤 위치를 자연스럽게 유지
      ...
      if (contentView) {
        const bottomDistance =
          state.scroll.action === 'preserve-footer' || state.scroll.action === 'restore-footer'
            ? state.scroll.bottomDistance
            : measureView(contentView).bottomDistance;
        state.scroll = { action: 'restore-footer', bottomDistance };
      } else {
        state.scroll = { action: 'idle' };
      }
      return;
  ...
  function renderToDOM(viewport: Viewport, activeInput: ActiveInput): void {
    const heightPx = toPx(viewport.height);
    if (heightPx !== state.snapshot.appHeight) {
      root.style.setProperty(cssVar.appHeight, heightPx);
      state.snapshot.appHeight = heightPx;
    }
    ...
    const hideFooter =
      ...
      (state.keyboard === 'closing' &&
        state.scroll.action !== 'restore-footer' &&
        state.scroll.action !== 'preserve-footer');
  ...
  function executeScrollIntent(
    ...
    switch (intent.action) {
      case 'align-content': {
        if (activeInput.region !== 'content' || !activeInput.editable) break;
        if (keyboardHeight < config.openThreshold && visualViewport) break;
      ...
      case 'preserve-footer': {
        if (activeInput.region === 'content') break;
        if (contentView) {
          const { maxScrollTop } = measureView(contentView);
          setScrollTop(contentView, maxScrollTop - intent.bottomDistance);
        }
        break;
      }
      ...
      case 'snap-bottom': {
        if (activeInput.region === 'content' || state.keyboard !== 'closed') break;
        if (contentView) {
          const { maxScrollTop } = measureView(contentView);
          setScrollTop(contentView, maxScrollTop);
        }
        state.scroll = { action: 'idle' };
        break;
      }
