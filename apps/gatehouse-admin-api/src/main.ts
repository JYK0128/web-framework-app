...
async function bootstrap(): Promise<void> {
  for (const databaseUrl of [env.DATABASE_URL, env.SERVICE_DATABASE_URL]) {
    if (!databaseUrl.startsWith('sqlite:')) continue;

    const dbFilePath = databaseUrl.replace(/^sqlite:\/{3}/, '');
