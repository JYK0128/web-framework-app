...
export class Migration20260813022904InitialSchema extends Migration {
  override up(): void | Promise<void> {
    ...
    this.addSql(`create table \`session\` (\`id\` text not null primary key, \`createdAt\` datetime not null, \`createdBy\` text null, \`updatedAt\` datetime not null, \`updatedBy\` text null, \`deletedAt\` datetime null, \`deletedBy\` text null, \`metadata\` json null, \`token\` text not null, \`user\` text null, \`expiresAt\` datetime null, \`ipAddress\` text null, \`userAgent\` text null, \`impersonatedBy\` text null, constraint \`session_user_foreign\` foreign key (\`user\`) references \`user\` (\`id\`) on delete cascade);`);
