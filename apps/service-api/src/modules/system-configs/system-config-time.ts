...
import type { SystemConfigSnapshot } from './system.context';

function getLocalTime(now: Date): TZDate {
  return TZDate.tz(DEFAULT_TIMEZONE, now);
}

export function isWithinOperatingHours(operation: SystemConfigSnapshot['operation'], now: Date): boolean {
...
export function getMaintenanceMessage(maintenance: SystemConfigSnapshot['maintenance'], now: Date): string | undefined {
