 ...
 *   user-web (browser) → user-api (JWT auth) → service-api (Machine internal guard)
 *
 * Requires both user-api and service-api to be running.
...
test.describe('Machine S2S Pipeline: User → Customers', () => {
  /**
   * Shared login helper: logs in as super-user and returns the auth cookie context.
   * Re-used across tests to avoid repeating login steps.
   */
  async function loginAsUser(request: APIRequestContext) {
  ...
  test('should list service-api customers via Machine auth after user login', async ({ request }) => {
    const headers = await loginAsUser(request);

    // Call user-api /api/v1/customers — which internally issues a machine token
    ...
    // caller should reflect the user's sub (actor propagation)
    expect(body).toHaveProperty('caller');
  });

  test('should retrieve a specific customer via Machine auth after user login', async ({ request }) => {
    const headers = await loginAsUser(request);
  ...
  test('should return 401 when calling /api/v1/customers without auth', async ({ request }) => {
    // Direct API call without any cookie/token — user-api AuthenticationGuard should reject
...
test.describe('Machine Security: Internal Endpoint Direct Access', () => {
  test('service-api /api/v1/internal/users must reject requests without a valid machine token', async ({
    ...
    // Direct call to service-api bypassing user-api — no machine token present
  ...
  test('service-api /api/v1/internal/users must reject a token with wrong audience', async ({
    ...
    // A token claiming aud=user-api (self) instead of service-api — audience mismatch
    const wrongAudToken = [
      'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9',
