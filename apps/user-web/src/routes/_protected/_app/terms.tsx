...
import { getTermsControllerGetUserTermsV1QueryKey, useTermsControllerCreateTermV1, useTermsControllerDeleteTermV1, useTermsControllerGetUserTermGroupsV1, useTermsControllerGetUserTermsV1, useTermsControllerPublishTermV1, useTermsControllerUpdateTermV1 } from '#/.generated/api/endpoints/terms/terms';
...
function TermsManagementPage() {
  ...
  const { data, isLoading } = useTermsControllerGetUserTermsV1(
    { page: 1, limit: 100 },
    { query: { staleTime: 30_000 } },
  );
  const groupsQuery = useTermsControllerGetUserTermGroupsV1({ query: { staleTime: 30_000 } });

  const terms = data?.data.items ?? [];
  const groups = groupsQuery.data?.data.items ?? [];

  const invalidate = async () => {
    await queryClient.invalidateQueries({ queryKey: getTermsControllerGetUserTermsV1QueryKey() });
  };
