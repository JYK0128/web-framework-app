...
export function createHealthRoute(options: HealthRouteOptions): Router {
  ...
  route.get('/health/ready', async (_req, res) => {
    const userApiReady = await isServiceReady(env.USER_API_URL);
    const checks = {
      userApi: userApiReady ? 'up' : 'down',
    };
    const isReady = !options.isShuttingDown() && checks.userApi === 'up';
