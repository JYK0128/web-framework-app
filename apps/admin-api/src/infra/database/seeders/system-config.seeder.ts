...
const DEFAULT_CONFIGS: Array<{ code: SystemConfigCode, description: string, value: Record<string, unknown> }> = [
  ...
  { code: SystemConfigCode.INQUIRY, description: '미응답 문의 감지, 답변 완료 후 자동 종료 시간 및 운영자 알림 웹훅 설정', value: { unansweredThresholdMinutes: 10, autoCloseHours: 72, notification: { enabled: false, type: 'SLACK', cooldownMinutes: 10, webhookUrl: '' } } },
  { code: SystemConfigCode.NOTIFICATION, description: '대고객 4대 채널(이메일 SMTP, 카카오톡/메신저, SMS 문자, 웹 푸시) 발송 설정', value: { email: { from: '', smtp: { host: '', port: 587, secure: false, user: '', pass: '' } }, messenger: { enabled: false, provider: 'KAKAO' }, sms: { enabled: false, provider: 'NHN_SMS' }, push: { enabled: false, provider: 'FCM', fcm: { projectId: '', clientEmail: '', privateKey: '' } } } },
  {
