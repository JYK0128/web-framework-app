...
export class AccountRecoveryService {
  ...
  async requestPasswordReset(email: string, phoneNumber: string) {
    ...
    if (user && user.profile?.phoneNumber === phoneNumber.trim()) {
      ...
      await this.kvStore.set(`user:password-reset:${challengeId}`, { userId: user.id, token } satisfies ResetRecord, 900);
      const link = `${process.env.USER_WEB_URL ?? 'http://localhost:15000'}/reset-password?challengeId=${challengeId}&token=${token}`;
      await this.sendEmail(user.email, link);
    }
  }

  async verifyPasswordReset(challengeId: string, token: string) {
    const record = await this.kvStore.get<ResetRecord>(`user:password-reset:${challengeId}`);
    return { isValid: Boolean(record && record.token === token) };
  }

  async resetPassword(challengeId: string, token: string, newPassword: string) {
    const record = await this.kvStore.getAndDelete<ResetRecord>(`user:password-reset:${challengeId}`);
  ...
  private async sendEmail(to: string, link: string) {
    ...
    await createTransport({ host, port: Number(SMTP_PORT), secure: process.env.SMTP_SECURE === 'true', auth: { user, pass } }).sendMail({ from: from ?? user, to, subject: '[User] 비밀번호 재설정', text: `비밀번호 재설정 링크: ${link}` });
  }
}
