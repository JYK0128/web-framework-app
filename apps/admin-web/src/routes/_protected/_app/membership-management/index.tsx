...
function MembershipManagementPage() {
  ...
  const openEditor = (membership?: MembershipItemDto) => {
    void openModal(MembershipEditor, { membership }).then((saved) => {
      if (saved) void queryClient.invalidateQueries({ queryKey: getMembershipsControllerListV1QueryKey() });
    });
  };
      ...
      >
            ...
            {memberships.map((membership) => (
              ...
              >
                <span className="flex items-center gap-2 font-semibold">
                  ...
                  {membership.label || membership.code}
                  {membership.isSystem && (
                    <span className="
        ...
        <SectionCard textSize="sm" title={selected ? `${selected.label || selected.code} (${selected.code})` : '멤버십을 선택하세요'} description={selected?.description || '멤버십 설명이 없습니다.'}>
          {selected && (
            <SectionCard.Actions>
              ...
              <Button
                ...
                "
                disabled={selected.isSystem || selected.customerCount > 0 || remove.isPending}
                onClick={() => void deleteMembership(selected)}
              >
          ...
          <SectionCard.Content className="
            grid content-start gap-4 overflow-hidden
          "
          >
            {selected
              ? (
                <div className="grid gap-4 rounded-lg border p-4">
                  <p className="text-sm font-semibold">멤버십 정보</p>
                  <dl className="grid gap-2 text-sm">
                    ...
                    <div className="flex justify-between gap-4">
                      <dt className="text-muted-foreground">적용 고객</dt>
                      <dd>
                        {selected.customerCount}
                        명
                      </dd>
                    </div>
                  </dl>
                  <div className="grid gap-2 border-t pt-3">
                    <p className="text-sm font-semibold">서비스 권한</p>
                      ...
                      ? (
                        <div className="flex flex-wrap gap-2">
                          {selected.permissions.map((permission) => (
                            <span
                              className="
                                rounded-md border bg-muted/40 px-2 py-1
                                font-mono text-xs
                              "
                              key={permission}
                            >
