  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <Modal.Content className="max-h-[calc(100vh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto]">
        <Modal.Header>
          <div className="flex items-start justify-between gap-3 pr-8">
            <div className="min-w-0">
              ...
              <Modal.Description>{currentRoom.userName} · {currentRoom.status === 'open' ? '대기' : currentRoom.status === 'in_progress' ? '상담 중' : '종료'}</Modal.Description>
            </div>
        ...
        <Modal.Body className="scroll-y">
          ...
          {!messages.isLoading && !messages.isError && (
            <div className="grid gap-3 py-2">
              {items.length === 0 && <p className="text-sm text-muted-foreground">아직 메시지가 없습니다.</p>}
        ...
        <Modal.Footer>
          <form.AppForm>
            <FormLayout id="support-message-form" onSubmit={() => void form.handleSubmit()} className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-end gap-2 sm:flex-1 [&>*]:col-span-1">
              <div className="min-w-0"><form.AppField name="content">{(field) => <field.Textarea aria-label="메시지" placeholder={isClosed ? '종료된 상담입니다.' : '답변을 입력해 주세요.'} rows={2} showError={false} disabled={isClosed || send.isPending} />}</form.AppField></div>
...
function MessageBubble({ message }: { message: SupportMessageItem }) {
  ...
  return (
    <div className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[82%] rounded-2xl border px-4 py-3 text-sm ${mine ? 'border-primary bg-primary text-primary-foreground' : 'bg-muted/40'}`}>
