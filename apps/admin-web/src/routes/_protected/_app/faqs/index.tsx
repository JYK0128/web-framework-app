...
function FaqManagementPage() {
  ...
  const openEditor = useCallback((faq?: FaqItemDto) => { void openModal(FaqEditorModal, { faq }); }, []);
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          <div className="flex justify-end gap-1" onClick={(event) => event.stopPropagation()}>
            {canUpdate && <Button type="button" variant="ghost" size="icon" aria-label="FAQ 수정" onClick={() => openEditor(row.original)}><Pencil className="size-4" /></Button>}
            {canDelete && <Button type="button" variant="ghost" size="icon" aria-label="FAQ 삭제" onClick={() => void handleDelete(row.original)}><Trash2 className="size-4 text-destructive" /></Button>}
          </div>
        ),
      }),
    ],
  ...
  return (
    <PageSection icon="circle-help" title="FAQ 관리" description="서비스에 공개되는 FAQ를 확인합니다.">
      <PageSection.Content className="
        grid grid-rows-[minmax(0,1fr)] gap-6 p-2
      "
      >
        <SectionCard textSize="sm" title="FAQ 목록" description="게시된 FAQ와 비게시 FAQ를 함께 확인할 수 있습니다.">
          {canCreate && (
            <SectionCard.Actions>
              <Button type="button" onClick={() => openEditor()}><Plus className="size-4" />FAQ 추가</Button>
            </SectionCard.Actions>
          )}
          ...
          "
          >
            <DataGridToolbar table={table} searchPlaceholder="질문 또는 답변 검색..." onReset={() => { setPage(1); setSearch(''); }} />
