...
// This is the documented initial password for newly created operators.
// eslint-disable-next-line sonarjs/no-hardcoded-passwords
const DEFAULT_OPERATOR_PASSWORD = '1q2w3e4r1@';
...
export function CreateOperatorModal({ open, onOpenChange, close }: CreateOperatorModalProps) {
  ...
  const form = useAppForm({
    ...
    validators: {
      onSubmit: OperatorsControllerCreateOperatorV1Body.omit({ password: true }).extend({
    ...
    onSubmit: async ({ value }) => {
      try {
        await createMutation.mutateAsync({
          data: { name: value.name.trim(), email: value.email.trim(), password: DEFAULT_OPERATOR_PASSWORD, role: value.role },
        });
      }
      catch (error) {
    ...
    >
      <Modal.Content size="md">
        <Modal.Header>
          ...
          <Modal.Description>새 운영자 계정을 생성합니다.</Modal.Description>
        </Modal.Header>
        <form.AppForm>
          <FormLayout
