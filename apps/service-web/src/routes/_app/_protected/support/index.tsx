...
function SupportPage() {
  ...
  const openRoom = useCallback((room: SupportRoomItem) => { void openModal(SupportRoomDetailModal, { room }); }, []);
  const table = useDataGrid({
    client: false,
    cursor: true,
    data: items,
    columns: [
      columnHelper.accessor('title', { header: '상담 제목', cell: ({ getValue }) => <span className="font-medium">{getValue()}</span> }),
      columnHelper.accessor('status', { header: '상태', cell: ({ getValue }) => statusLabels[getValue()] }),
  ...
  return (
    <div className="size-full px-6 py-8 md:px-8">
      <PageSection icon="messages-square" title="고객지원" description="챗봇과 상담원에게 도움을 요청하고 대화할 수 있습니다.">
        ...
        <PageSection.Content className="mx-auto grid size-full min-w-0 max-w-5xl grid-rows-[minmax(0,1fr)] gap-6 pt-2">
          <SectionCard textSize="sm" title="내 상담" description="상담방을 열어 메시지를 확인하고 이어서 대화할 수 있습니다.">
            <SectionCard.Content className="grid h-full min-w-0 grid-rows-[minmax(0,1fr)] overflow-hidden p-4">
