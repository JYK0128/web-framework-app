...
export function SupportRoomDetailModal({ room, open, onOpenChange, close }: SupportRoomDetailModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape' && !send.isPending && !update.isPending) close?.(); };
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <Modal.Content className="max-h-[calc(100vh-2rem)] grid-rows-[auto_minmax(0,1fr)_auto]">
        <Modal.Header>
        ...
        <Modal.Body className="scroll-y">
          ...
          {!messages.isLoading && !messages.isError && <div className="grid gap-3 py-2">{items.length === 0 && <p className="text-sm text-muted-foreground">아직 메시지가 없습니다.</p>}{items.map((message) => <MessageBubble key={message.id} message={message} />)}</div>}
        </Modal.Body>
        <Modal.Footer>
          <form.AppForm>
            <FormLayout id="support-room-message-form" onSubmit={() => void form.handleSubmit()} className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-end gap-2 sm:flex-1 [&>*]:col-span-1">
...
function MessageBubble({ message }: { message: SupportMessageItem }) {
  ...
  return <div className={`flex ${mine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] rounded-2xl border px-4 py-3 text-sm ${mine ? 'border-primary bg-primary text-primary-foreground' : 'bg-muted/40'}`}><div className="mb-1 text-xs font-semibold opacity-75">{message.senderName}</div><p className="whitespace-pre-wrap wrap-break-word leading-6">{message.content}</p><div className="mt-1 text-right text-[10px] opacity-60">{new Date(message.createdAt).toLocaleString('ko-KR')}</div></div></div>;
}
